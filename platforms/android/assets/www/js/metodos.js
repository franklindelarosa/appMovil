...
var verificarLogin = function (result){
    // console.log(result);
    if(result.status === "ok"){
        if(result.key[0].estado === '9'){
            $$('#login div.form').find(':not(button)[type="password"]').val('');
            // var html_info = '<br><p class="centrar"><img src="images/info.png"/></p><br><small class="centrar">Tu cuenta aún no ha sido verificada, revisa tu correo para completar el registro</small>';
            // Lungo.Notification.html(html_info, "Cerrar");
            // $$('div.window .confirm.show small').style('display: block');
            Lungo.Notification.confirm({
                icon: 'info-sign',
                title: 'Tu cuenta aún no ha sido verificada, te gustaría volver a recibir el correo de verificación?',
                description: '',
                accept: {
                    icon: 'ok',
                    label: 'Si',
                    callback: function(){
                        ...
                        var url = direccionBase+"site/reenviar-correo";
                        Lungo.Service.post(url, {correo: result.key[0].correo}, function(result){
                            if(result.status === 'ok'){
...
var verificarRegistro = function (result){
    if(result.status === 'ok'){
        if(result.key === 'no'){
            ...
            if(result.correo){
                Lungo.Notification.success("Correcto", "Se ha completado el registro, revisa tu correo para verificar la cuenta", "ok", function(){return});
...
// esta función cierra sesión del usuario actual
