...
$$('#editar-perfil').on('unload', function(event) {
    setTimeout(function(){$$('#registrar div.form').find(':not(button)[id]').val('');}, 350);
});
...
$$('#btn_reestablecer').on('singleTap', function(event) {
    event.preventDefault();
    var url = direccionBase+"site/reestablecer-contrasena";
    document.activeElement.blur();
    Lungo.Notification.show();
    var correo = $$('#correo_reestablecimiento').val();
    if(correo === ''){
        Lungo.Notification.error("Error", "Debes escribir el correo de la cuenta a reestablecer", "remove", function(){return});
    }else{
        Lungo.Service.post(url, {correo: correo}, function(result){
            if(result.status === 'ok'){
                $$('#correo_reestablecimiento').val('');
                Lungo.Router.section('login');
...
$$(document).on('singleTap', '#cerrar-sesion', function(event) {
    cerrarSesion();
    $$('#edit_contrasena').removeAttr('disabled');
    $$('#fieldset_edit_contrasena').show();
});

$$('nav > a[data-view-menu="opciones-menu"]').on('singleTap', function(event) {
    var menu = $$('nav#opciones-menu');
...
$$(document).on('singleTap', '#eliminar-cuenta', function(event) {
    Lungo.Notification.confirm({
        icon: 'exclamation-sign',
        ...
        accept: {
            ...
            callback: function(){
                Lungo.Notification.show();
                var url = direccionBase+"usuario/eliminar-cuenta?access-token="+localStorage["_chrome-rel-back"];
